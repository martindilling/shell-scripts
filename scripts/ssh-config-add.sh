...
version="1.0.0"
#
# ./ssh-config-add.sh --host="frink" --hostname="192.168.111.87" --identity="~/.ssh/id_rsa" --user="root" --extras="GSSAPIAuthentication no,Test: yes"
#
# ##################################################
...
#USER_CONFIG=$(cat config | awk "/${AUTO_HEADER}/ {exit} {print}")
#AUTO_CONFIG=$(cat newconfig)

# Config file to update
sshConfigFile="$HOME/.ssh/config"

# Array with information from options
lines=(
    "Host: ${host}"
    "HostName: ${hostName}"
...
# If extras are set as a commaseperated string, add them to the lines array
if [[ ${extras} ]]; then
    IFS=, read -r -a extraLines <<<"${extras}"
    lines=("${lines[@]}" "${extraLines[@]}")
fi


# Add the header if it doesn't exist
if ! grep -q ${autogeneratedHeader} ${sshConfigFile}; then
    alert_info "Creating header line"
    echo -e "\n\n" >> ${sshConfigFile}
    echo "${autogeneratedHeader}" >> ${sshConfigFile}
fi

# Create string from the array with lines
entry="`printf '%s\n' "${lines[@]}"`"

# Add the entry only if it can't be found in the file
if pcregrep -M -q "${entry}" ${sshConfigFile}; then
    alert_notice "Entry already added"
else
    alert_info "Adding ssh config entry to ${sshConfigFile}"
    echo -e "\n" >> ${sshConfigFile}
    echo "${entry}" >> ${sshConfigFile}
    alert_success "All done..."
fi
