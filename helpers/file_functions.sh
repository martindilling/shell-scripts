...
function write_to_file () {
    if is_empty "${1}"; then
        alert_die "File must be given as first parameter to 'write_to_file'."
    fi
    if is_not_file "${1}"; then
        alert_die "File given as first parameter to 'write_to_file' must exists and be a file."
    fi
    if is_empty "${2}"; then
        alert_die "String must be given as second parameter to 'write_to_file'."
    fi
...
function clean_autogenerated_from_file () {
    ...
    write_to_file "${1}" "${userPartOfFile}"
